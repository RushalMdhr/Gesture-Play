...
with vision.GestureRecognizer.create_from_options(options) as recognizer:
    ...
    while True:
        ...
        if result.gestures:
            ...
            # Draw landmark dots for every detected hand
            ...
            if len(result.hand_landmarks) == 2:
                ...
                # First pass: try to trust MediaPipe's own Left/Right labels
                ...
                # FIX: MediaPipe sometimes labels BOTH hands as the same side
                # (this is what was crashing your script). When that happens,
                # fall back to using screen position instead: whichever hand
                # has the smaller x-coordinate is "left" on screen.
                if not (got_left and got_right):
                    hand_a = result.hand_landmarks[0]
                    hand_b = result.hand_landmarks[1]
                    if hand_a[9].x <= hand_b[9].x:
                        left, right = hand_a, hand_b
                    else:
                        left, right = hand_b, hand_a
                    got_left, got_right = True, True
                    print("Handedness labels were ambiguous — used screen position instead")

                print("__________________________ left : ", left[9].x, right[9].x)

                mid_X, mid_Y, left_X, left_Y, right_X, right_Y = getMidPoint(right, left)
                mid_X, left_X, right_X, mid_Y, left_Y, right_Y = (
                    int(mid_X * frame.shape[1]), int(left_X * frame.shape[1]), int(right_X * frame.shape[1]),
                    int(mid_Y * frame.shape[0]), int(left_Y * frame.shape[0]), int(right_Y * frame.shape[0])
                )
